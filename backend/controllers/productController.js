...
const createProduct = async (req, res) => {
  const { name, category, gsm, dimensions, material, washCare, images, featured, visible } = req.body;

  try {
    if (!name || !category || !gsm || !dimensions) {
    ...
    const product = new Product({
      ...
      dimensions,
      material,
      washCare: Array.isArray(washCare) ? washCare : washCare ? washCare.split(",").map(i => i.trim()) : [],
    ...
    const populated = await Product.findById(createdProduct._id).populate("category");
...
const updateProduct = async (req, res) => {
  ...
  try {
    ...
    if (washCare !== undefined) {
      product.washCare = Array.isArray(washCare) ? washCare : washCare ? washCare.split(",").map(i => i.trim()) : [];
    }
